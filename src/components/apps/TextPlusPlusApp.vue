...
async function save() {
	...
	try {
		await fileSystemStore.editFile(
			fileToSave.value.path || '/home',
			fileToSave.value.name,
			new Blob([fileToSave.value.content], { type: fileToSave.value.mimeType }),
			fileToSave.value.mimeType,
			true,
		);
	} catch (error) {
...
async function triggerSaveFromDialog() {
	...
	try {
		await fileSystemStore.editFile(
			...
			fileToSave.value.name,
			new Blob([fileToSave.value.content || ''], { type: fileToSave.value.mimeType }),
			fileToSave.value.mimeType || 'text/plain',
			true,
		);
		closeDialog();
	} catch (error) {
		...
		<md-dialog :open="dialogOpened" ref="saveDialog" @closed="dialogOpened = false">
			...
			<div class="save-dialog-content" slot="content">
				<md-outlined-text-field
					v-model="fileToSave.name"
					label="Filename"
				></md-outlined-text-field>
				<div class="location-container">
					<md-outlined-button id="file-location" @click="toggleMenu()"
						>Location</md-outlined-button
					>
					<p>{{ fileToSave.path }}</p>
				</div>
				<md-filled-button id="file-location" @click="triggerSaveFromDialog()"
					>Save</md-filled-button
				>
				<p v-if="saveError" class="save-error">{{ saveError }}</p>
				<md-menu
					class="file-system-menu"
					ref="fileSystemMenu"
				...
				>
					...
					<DirectoryMenuItem
						:nodes="fileSystemFromRoot?.children"
						@select="handleSelect"
					/>
				</md-menu>
			</div>
